...
## Confirmed temporary test-domain rebuild

The confirmed central temporary default is now `http://fleetiq.fwh.is/`. It remains overrideable through `FLEETIQ_SITE_URL`. No HTTPS redirects were added or SSL availability assumed. The 40-file `build/public-preparation/` is ready for the requested HTTP website test upload with mail unconfigured. The original domain-free preparation copy was preserved beside it in an ignored build backup; upload only the current package contents. Exact current manifest: `build/package-manifest.txt`.

Validation against the rebuilt package: all 27 packaged PHP files and the updated builder lint clean; 48 security checks pass; 36 HTTP/form checks pass with delivery disabled and again with the normal `mail` transport but sender missing (loopback-only SMTP safety setting; no external email). A separate package audit passes 854 assertions: exact 40-file allowlist and source hashes; no localhost/reserved URLs or Windows paths in package/output; 326 link references, six assets, all 12 canonical/OG routes, all 12 sitemap entries, crawl-allowed robots, Privacy/Terms, direct/nested 404, Book a Demo targets, unchanged recipient `iancrofts@live.co.uk`, and unset sender/delivery readiness. Chrome reran 468 actual navigation clicks against the package at desktop/mobile widths with no failures. No layout changes were made; the earlier responsive results remain applicable. `git diff --check` passes.

Evidence: `%TEMP%/fleetiq-test-domain-qa/` (validation/HTTP scripts, navigation results and private local server logs). Remaining checks: actual host rules/PHP settings, authorised sender and real-host mail receipt, SSL confirmation and real-browser/device checks after upload. No commit, push or upload occurred. The older unknown-domain/package-blocked notes below are superseded by this entry and `DEPLOYMENT.md`.
